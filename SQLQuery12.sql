...
/* В этом запросе мы считаем ежемесячные доходы компании за выбранный период. 
Сначала задаём диапазон дат. Затем в первой части вычисляем сумма дохода от оплаченных аренд велосипедов, во второй — сумма дохода от ремонтов. 
После этого результаты объединяются, чтобы показать все месяцы, где были либо аренды, либо ремонты. 
В итоговой таблице для каждого месяца выводим доход от аренды, от ремонта и общий доход, упорядоченный по году и месяцу.*/
